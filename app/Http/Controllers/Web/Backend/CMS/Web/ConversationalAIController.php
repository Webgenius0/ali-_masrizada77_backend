...
class ConversationalAIController extends Controller
{
    ...
    public function __construct(CmsService $cmsService)
    {
        ...
        $this->page = PageEnum::ConversationalAI;
        $this->section = SectionEnum::INTRO;
        $this->uploadPath = 'cms/ConversationalAI';
    }
    ...
    public function index(Request $request)
    {
        ...
        return view("backend.layouts.cms.conversationalAI.conversationalAI", [
            "data" => $data,
    ...
    public function content(CmsRequest $request)
    {
        try {
            ...
            if ($request->hasFile('bottom_video')) {
                ...
                $image4 = Helper::fileUpload($request->file('bottom_video'), $this->uploadPath . '/videos', 'sec3_vid_' . time());
            }
            ...
            // ৪. সেকশন ৫ লুপ আইটেম ও ইমেজ
            $sec5_items = $request->sec5_items ?? [];
            for ($i = 0; $i < 4; $i++) {
                $fileKey = "sec5_item_image_$i";
                if ($request->hasFile($fileKey)) {
                    if (!empty($sec5_items[$i]['old_image'])) {
                        $this->removeOldFile($sec5_items[$i]['old_image']);
                    }
                    $sec5_items[$i]['image'] = Helper::fileUpload($request->file($fileKey), $this->uploadPath . '/items', "sec5_item_{$i}_" . time());
                } else {
                    $sec5_items[$i]['image'] = $sec5_items[$i]['old_image'] ?? ($section->metadata['sec5_items'][$i]['image'] ?? null);
                }
                unset($sec5_items[$i]['old_image']);
            }
            ...
            $metadata = [
                ...
                'sec5_main_title'   => $request->sec5_main_title,
                'sec5_main_desc'    => $request->sec5_main_desc,
                'sec5_items'        => $sec5_items,
            ];
